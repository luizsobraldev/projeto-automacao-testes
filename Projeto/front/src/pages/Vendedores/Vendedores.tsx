...
const Vendedores: React.FC = () => {
  ...
  const fetch = async () => {
    setLoading(true)
    const res = await api.get(`/vendedores?inativo=${inativo}`)
    setLista(res.data); setLoading(false)
  }
  ...
  const handleSave = async () => {
    setErro('')
    try {
      if (editing) await api.put(`/vendedores/${editing.id}`, form)
      else await api.post('/vendedores', form)
      setShowModal(false); fetch()
    } catch (e: any) { setErro(e.response?.data?.detail || 'Erro ao salvar') }
  }

  const toggleAtivo = async (item: any) => {
    if (item.ativo) await api.patch(`/vendedores/${item.id}/inativar`)
    else await api.patch(`/vendedores/${item.id}/reativar`)
    fetch()
  }
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 20 }}>
        <h2>Vendedores</h2>
        <button onClick={() => openModal()} style={btnPrimary}>+ Novo Vendedor</button>
      </div>
        ...
        {loading ? <p style={{ padding: 16 }}>Carregando...</p> : (
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead><tr style={{ background: '#1976d2', color: '#fff' }}>
              {['Nome', 'Email', 'Telefone', 'Descrição', 'Admin', 'Ações'].map(h => <th key={h} style={thStyle}>{h}</th>)}
            </tr></thead>
            <tbody>{lista.map((item, i) => (
              <tr key={item.id} style={{ background: i % 2 === 0 ? '#fff' : '#f9f9f9' }}>
                ...
                <td style={tdStyle}>{item.isAdmin ? '✅' : '—'}</td>
      ...
      {showModal && (
        <div style={overlayStyle}>
          <div style={modalStyle}>
            <h3 style={{ marginBottom: 16 }}>{editing ? 'Editar Vendedor' : 'Novo Vendedor'}</h3>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              {[{ key: 'nome', label: 'Nome *' }, { key: 'email', label: 'Email *', type: 'email' }, { key: 'telefone', label: 'Telefone' }, { key: 'descricao', label: 'Descrição' }].map(c => (
                <div key={c.key}>
...
const modalStyle: React.CSSProperties = { background: '#fff', borderRadius: 12, padding: 32, minWidth: 560, maxWidth: '90vw', boxShadow: '0 8px 32px #0004' }
